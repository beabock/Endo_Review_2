#!/usr/bin/env Rscript
# BMB 2026-06-17
# Same as 03_geographic_bias_mapping.R but groups EU countries together.
...
INTERACTIVE_MAP_FILE <- file.path(RESULTS_DIR, "interactive_study_density_eu_grouped.html")
BIAS_METRICS_FILE <- file.path(RESULTS_DIR, "geographic_bias_metrics_eu_grouped.csv")
CONTINENTAL_FILE <- file.path(RESULTS_DIR, "continental_breakdown_eu_grouped.csv")
BIAS_HEATMAP_FILE <- file.path(RESULTS_DIR, "geographic_bias_heatmap_eu_grouped.png")
...
eu_data <- country_data %>% filter(iso_a3 %in% eu_iso3)
non_eu_data <- country_data %>% filter(!iso_a3 %in% eu_iso3)

eu_aggregated <- tibble(
  iso_a3 = "EUU",
  country_name = "European Union",
  centroid_lon = 9.0,   
  centroid_lat = 50.0,
  study_count = sum(eu_data$study_count, na.rm = TRUE),
  gdp_year = max(eu_data$gdp_year, na.rm = TRUE),
  gdp_current_usd = sum(eu_data$gdp_current_usd, na.rm = TRUE)
) %>%
  mutate(
    gdp_log10 = ifelse(!is.na(gdp_current_usd) & gdp_current_usd > 0, 
                       log10(gdp_current_usd), NA_real_)
...
country_data <- bind_rows(non_eu_data, eu_aggregated)
...
world_eu <- world %>% filter(iso_a3 %in% eu_iso3) %>%
  mutate(iso_a3 = "EUU", name = "European Union", continent = "Europe") %>%
  group_by(iso_a3, name, continent) %>%
  summarise(geometry = st_union(geometry), .groups = "drop")

world_non_eu <- world %>% filter(!iso_a3 %in% eu_iso3)
world <- bind_rows(world_non_eu, world_eu)
